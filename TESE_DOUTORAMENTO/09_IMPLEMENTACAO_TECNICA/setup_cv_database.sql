-- Setup script for Computer Vision database tables
-- Run this script to add computer vision functionality to your existing database
...
CREATE INDEX IF NOT EXISTS idx_detections_analysis_id ON video_detections(analysis_id);
CREATE INDEX IF NOT EXISTS idx_detections_frame ON video_detections(frame_number);
CREATE INDEX IF NOT EXISTS idx_detections_class ON video_detections(object_class);
CREATE INDEX IF NOT EXISTS idx_detections_timestamp ON video_detections(timestamp_seconds);
...
-- Create indexes
CREATE INDEX IF NOT EXISTS idx_video_metrics_session_id ON video_metrics(session_id);
CREATE INDEX IF NOT EXISTS idx_video_metrics_analysis_id ON video_metrics(analysis_id);
...
BEGIN
    ...
    GROUP BY session_id_param;
END;
$$ LANGUAGE plpgsql;

-- Insert sample data to verify tables work
INSERT INTO video_analysis (analysis_id, session_id, video_path, analysis_type, status) 
VALUES ('test-analysis-123', 1, '/test/video.mp4', 'full', 'completed')
ON CONFLICT (analysis_id) DO NOTHING;

-- Verify tables were created successfully
SELECT 'video_analysis' as table_name, COUNT(*) as row_count FROM video_analysis
UNION ALL
SELECT 'video_detections' as table_name, COUNT(*) as row_count FROM video_detections
UNION ALL
SELECT 'video_metrics' as table_name, COUNT(*) as row_count FROM video_metrics;
