#!/usr/bin/env python3
"""Fix syntax error in AthleteDetail.jsx - corrupted import statement"""
...
print("🔧 Fixing AthleteDetail.jsx syntax error...")
print(f"   Current first line: {repr(content.split(chr(10))[0])}")

# The file appears to be corrupted with an incomplete import statement
# Let's recreate the proper imports and structure

fixed_content = '''import { useState, useEffect } from 'react'
        ...
        <div className="px-6 py-5 border-b border-gray-200">
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-2">
              ...
              >
                Treino ({metrics?.sessions?.filter(s => s.tipo === 'treino').length || 0})
              </button>
              <button
              ...
              >
                Jogos ({metrics?.sessions?.filter(s => s.tipo === 'jogo').length || 0})
              </button>
            </div>
          </div>
        </div>
              ...
              {filteredSessions.map((session) => (
                <tr key={session.sessao_id} className="hover:bg-gray-50">
                  ...
                  <td className="whitespace-nowrap px-3 py-3 text-sm text-gray-900 text-right">
                    {session.distancia_total ? `${Math.round(session.distancia_total)}m` : '-'}
                  </td>
                  <td className="whitespace-nowrap px-3 py-3 text-sm text-gray-900 text-right">
                    {session.velocidade_max ? `${session.velocidade_max.toFixed(1)} km/h` : '-'}
                  </td>
                  <td className="whitespace-nowrap px-3 py-3 text-sm text-gray-900 text-right">
                    {session.rpe || '-'}
                  </td>
...
print("✅ Fixed AthleteDetail.jsx syntax error:")
print("   • Recreated proper import statements")
print("   • Fixed unterminated string constant")
print("   • Restored complete component structure")
print("   • Maintained wellness and physical tests integration")
print("   • Added comprehensive profile data loading")
