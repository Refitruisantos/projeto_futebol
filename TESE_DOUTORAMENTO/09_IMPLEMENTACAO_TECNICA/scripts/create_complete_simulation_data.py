#!/usr/bin/env python3
"""Create comprehensive simulation data including opponent data, decelerations, and sleep tracking"""
...
cursor.execute("SELECT id FROM dados_gps")
...
for record in gps_records:
    ...
    cursor.execute("""
        ...
        WHERE id = %s
    """, (num_high_decel, max_decel, avg_decel, gps_id))
...
for record in wellness_records:
    ...
    bedtime_hour = random.randint(22, 24)
    ...
    sleep_onset_hour = bedtime_hour if bedtime_min + sleep_onset_delay < 60 else bedtime_hour + 1
    sleep_onset_min = (bedtime_min + sleep_onset_delay) % 60
...
# Get game sessions without opponent data
cursor.execute("SELECT id FROM sessoes WHERE tipo = 'jogo' AND (adversario IS NULL OR adversario = '')")
